...
def rough_info(content: str) -> tuple[str, str]:
    ...
    info = f'<details class="rough-info" open><summary>处理信息</summary><dl>{"".join(rows)}</dl></details>' if rows else ""
...
.rough-info{margin:.8rem 0 .4rem;border:1px solid var(--line);border-radius:8px;background:var(--panel)}
...
class ReviewService:
    ...
    def _form_card(self, rough: RoughBinding, nonce: str, *, wiki_path: str = "", candidate: str = "", root: Path | None = None, action_query: str = "", heading: str = "原文") -> str:
        ...
        return f"""<article><h2>{html.escape(heading)}</h2><pre>{html.escape(rough_info(rough.content)[1])}</pre>{rough_info(rough.content)[0]}
