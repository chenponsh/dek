...
class ReviewWorkflowTests(unittest.TestCase):
    ...
    def test_raw_text_folds_the_draft_metadata_into_a_chinese_processing_info_block(self):
        ...
        item = next(item for item in self.service.list_items() if item.path == "ingestion/rough/pending.md")
        page = self.service.render_item("opaque-session", item.identity).decode("utf-8")
        block = page.split('<details class="rough-info"><summary>处理信息</summary><dl>', 1)[1].split("</dl></details>", 1)[0]
        self.assertEqual(re.findall(r"<dt>([^<]+)</dt>", block), ["发布日期", "入库日期", "来源", "状态", "目标位置"])
        self.assertIn("<dt>发布日期</dt><dd>2026-03-16</dd>", block)
        ...
        for hidden in ("source_item_key", "sha256:item-version", "recommended_tags", "reviewed_at", "ingested_at", "published_date", "status: pending_review", "[[source/"):
            self.assertNotIn(hidden, page.split("<form", 1)[0], hidden)
        self.assertIn("</details><pre>## 新增问答", page)
