...
**可以做（低风险，先告诉用户你要做什么）**
- **摄入时合并“同问题同解答”的重复条目（2026-09-21 已做，仅限同一次运行内）**：同一次抓取里，第二个栏目的行若与本次已生成的草稿问题+解答完全相同，就不再生成新草稿，而是在第一份草稿的 `source:` 行追加第二个来源链接（`"[[a]] [[b]]"`），并在报告里记 `rough_also_sources: {草稿: [来源]}`。检查规则的改动（用户 2026-09-21 同意）：`cli.validate_report_invariants` / `validate_automatic_plan` 把 `rough_also_sources` 算作覆盖；`audit.py` 里草稿的“身份”改为它 `source:` 行里的每一个来源，且 also-sources 只有在草稿文件真的写了该来源时才算覆盖（否则仍报缺草稿）；审核页（`web/review.py`）能显示/解析多个来源。**没做**：跨天/跨次运行的重复（第一份草稿已提交，不再回头改）——这类仍是两份草稿，审核时拒绝其中一份即可；已有的 3 组待审重复和已上线的 0102-0012/0013 也保持原样。
