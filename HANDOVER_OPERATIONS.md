...
**可以做（低风险，先告诉用户你要做什么）**
- **摄入时合并“同问题同解答”的重复条目**（未做）：CDE 把同一问题放在“常见一般性技术问题”和“受理共性问题”两个栏目，各生成一条草稿（0102-0012/0013 已是这样）。机器人索引已在加载时合并（`qa/dek_qa/index.py` `_dedupe_documents`），网站仍是两条。要在摄入时合成一条，不能只改 `stage_source_rows`：`validate_report_invariants` 和 `audit.py` 要求每个 `updated_with_new` 的来源都有自己的草稿（`rough_sources` 是一对一），得同时改这两处，并让草稿的 `source:` 带两个链接（`web/site.py` 的批准判断、机器人的来源解析都已支持多个链接）。
